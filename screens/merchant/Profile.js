...
export default class Profile extends Component {
  ...
  _saveProfile(){
    ...
    const __this = this;
    attestation.tx("save",[issuer.address,profile_name,image,ipfs]).then(function(){
      __this.refresh();
    })
  }
  refresh(){
    const __this =this;
    attestation.query("profile",[issuer.address]).then(function(value){
      if (value.isSome ==true){
        let j = value.unwrap();
        Utils._getImage(j.ipfs,j.image).then(function(image_uri){
          __this.setState({image_uri,profile_name:hexToString(j.profile_name),ipfs:hexToString(j.ipfs)})
        })
      } 
    })
  }
  ...
  render(){
    const {image,image_uri,profile_name}  =this.state;
    const __this = this;
    return(<View>
      <Picker
